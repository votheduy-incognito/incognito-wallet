...
export const apiGetKyberQuote = async ({ inputToken, outputToken, amount }) => {
  ...
  if (bestRate) {
    try {
      maxAmountOut = BigNumber(bestRate?.MaxAmountOut || rates?.MaxAmountOut)
        .dividedToIntegerBy(1)
        .toNumber();
      expectAmount = bestRate?.RatioTrade || rates?.RatioTrade;
      erc20Fee = BigNumber(bestRate?.PrvFee || rates?.PrvFee)
        .dividedToIntegerBy(1)
        .toNumber();
  ...
  const data = {
    maxAmountOut, //pDecimals
    erc20Fee, //pDecimals
    expectAmount,
    protocol,
    dAppAddress,
  };
  console.debug('DATA_QUOTE', data);
  return data;
};
