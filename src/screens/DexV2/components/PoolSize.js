...
const PoolSize = ({ inputToken, outputToken, pair }) => {
  ...
  if (pair.length === 1) {
    const inputPool = pair[0][inputToken.id];
    const outputPool = pair[0][outputToken.id];
  ...
  const inputPool1 = pair[0][inputToken.id];
  const outputPool1 = pair[0][COINS.PRV_ID];

  const formattedInputPool1 = formatUtil.amount(inputPool1, inputToken.pDecimals, true);
  const formattedOutputPool1 = formatUtil.amount(outputPool1, COINS.PRV.pDecimals, true);

  const inputPool2 = pair[1][COINS.PRV_ID];
  const outputPool2 = pair[1][outputToken.id];

  const formattedInputPool2 = formatUtil.amount(inputPool2, COINS.PRV.pDecimals, true);
  const formattedOutputPool2 = formatUtil.amount(outputPool2, outputToken.pDecimals, true);

  if (!formattedInputPool1 || !formattedOutputPool1 || !formattedInputPool2 || !formattedOutputPool2) {
    return empty;
  }
